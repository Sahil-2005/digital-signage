...
def update_viewer_data(viewer_count, start_time, video_id):
    # Send a POST request to update the viewer data for the specific video
    url = f"http://192.168.88.152:4000/update-viewer-data/{video_id}"
    payload = {
    ...
    if response.status_code == 200:
        print(f"Viewer data for video {video_id} updated successfully.")
    else:
        print(f"Failed to update viewer data for video {video_id}.")



def get_current_video_info():
    # Send a GET request to the server to get information about the current video
    url = "http://192.168.88.152:4000/current-video-info"
